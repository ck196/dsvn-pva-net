...
def vis_detections(im, class_name, dets, thresh=0.3):
    ...
    for i in xrange(np.minimum(10, dets.shape[0])):
        ...
        if score > thresh:
            ...
            cv2.putText(im, class_name, (int(bbox[0]), int(bbox[1]) + 15), cv2.FONT_HERSHEY_DUPLEX, 0.5, (255,255,0) , 1)
...
def pva_detect(net, im_path, conf_thresh=0.5, classes=None):
    """Test a Fast R-CNN network on an image database."""
    ...
    im = cv2.imread(im_path) # need try catch
    ...
    frame_name, extend = im_path.split("/")[-1].split(".")
    annotate_response = AnnotateResponse(annotation_array)
    responses = Responses(frame_name, annotate_response)
    responses.image_depth = channels
