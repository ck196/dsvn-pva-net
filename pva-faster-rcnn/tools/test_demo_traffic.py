...
if __name__ == '__main__':
    ...
    caffe.set_mode_gpu()
    caffe.set_device(0)
    net = caffe.Net("output/test_traffic_light.prototxt", "rcnn/traf/pvanet_frcnn_384_iter_190000.caffemodel", caffe.TEST)
    ...
    CLASSES = ('__background__',
               'TRAFFIC_LIGHT', 'TRAFFIC_SIGN', 'PEDESTRIAN_CROSSING_ZONE')

    with open("traffic_v2.txt", "r") as filereader:
        lines = filereader.readlines()
        for line in lines:
            line = line.replace("\n","")
            print line
            pva_detect(net, line, 0.7, classes=CLASSES, vis=False, save=True, color=COLORS)
            #time.sleep(0.5)
